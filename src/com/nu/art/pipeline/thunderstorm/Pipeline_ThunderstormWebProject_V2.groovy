...
class Pipeline_ThunderstormWebProject_V2<T extends Pipeline_ThunderstormWebProject_V2>
        extends Pipeline_ThunderstormWebApp<T> {

    public Var_Env Env_DeployToEnv = new Var_Env("DEPLOY_TO_ENV")
    public Var_Env Env_GitCloneBranch = new Var_Env("GIT_CLONE_BRANCH")

    ProjectGitConfig gitConfig
    def envProjects = [:]
    String slackChannel

    Pipeline_ThunderstormWebProject_V2(String name, String slackChannel, Class<? extends WorkflowModule>... modules) {
        super(name, modules)
    ...
    protected void init() {
        ...
        String branch = Env_GitCloneBranch.get() ?: env
        env = env ?: branch
